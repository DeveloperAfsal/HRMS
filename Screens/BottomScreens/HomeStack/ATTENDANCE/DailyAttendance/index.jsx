...
import { ActivityIndicator, Alert, Linking, PermissionsAndroid, Platform, RefreshControl, ScrollView, Text, TextInput, TouchableOpacity, View } from "react-native";
...
const DailyAttendance = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                'https://officeinteriorschennai.com/api/public/api/dailyattendancelist',
                {
                    dailyattendancelist: data.userrole,
                    dailyloginempid: data.userempid
                },
                {
                    headers: {
    ...
    const renderTableRows = () => {
        return visibleData.map((item, index) => (
            <Row key={index}
                ...
                <Text style={styles.cellText}>{item.emp_present == null ? '-' : item.emp_present}</Text>,
                <Text style={styles.cellText}>{item.emp_late == null ? '-' : item.emp_late}</Text>,
                <Text style={styles.cellText}>{item.emp_permission == null ? '-' : item.emp_permission}</Text>,
                <Text style={styles.cellText}>{item.emp_onduty == null ? '-' : item.emp_onduty}</Text>,
                <Text style={styles.cellText}>{item.checkout_total_hours == null ? '-' : item.checkout_total_hours}</Text>]}
                style={styles.row}
                textStyle={styles.rowText}
                widthArr={[100, 150, 120, 120, 100, 100, 100, 100, 100]}
            />
        ));
    };
    ...
    const exportToPDF = async () => {
        const tableHead = ['S.No', 'Name', 'Date', 'In Time', 'Out Time', 'P/A/HL', 'LA', 'PR', 'Total Hours'];
        const tableData1 = filteredData.map((item, index) => [
            index + 1,
            item.first_name || '-',
            item.checkin_time ? item.checkin_time.slice(0, 10) : '-',
            item.checkin_time ? item.checkin_time.slice(10, 19) : '-',
            item.checkout_time ? (item.checkout_time == '00:00' ? '00:00' : item.checkout_time.slice(10, 19)) : '-',
            item.emp_present || '-',
            item.emp_late || '-',
            item.emp_permission || '-',
                        ...
                        <ScrollView horizontal>
                            ...
                            <View style={styles.tableContainer}>
                                <Table borderStyle={styles.tableBorder}>
                                    <Row
                                        data={['S.No', 'Name', 'Check In Time', 'Check Out Time', 'Present/Absent', 'Late', 'Permission', 'On duty', 'Total Hours']}
