...
import { View, Text, ScrollView, TouchableOpacity, ActivityIndicator, Alert, TextInput, RefreshControl, Linking } from 'react-native';
...
import EyeIcon from '../../../../../assets/EPK CRM Icons/EyeOpen.svg';
...
const EventList = ({ navigation }) => {
    ...
    const fetchTableData = async () => {
        setLoading(true);
        const apiUrl = 'https://officeinteriorschennai.com/api/public/api/event_list';
        const requestData = {
            eventlistroleid: data.userrole,
            eventlistempid: data.userempid,
        };
        try {
    ...
    // Handler navigate EDIT & VIEW

    const handleEdit = (item) => {
        navigation.navigate('Edit Event', { Id: item.id });
    }

    const handleView = (item) => {
        navigation.navigate('View Event', { Id: item.id });
    }
        ...
        Alert.alert(
            ...
            [
                ...
                },
                {
                    ...
                    onPress: async () => {
                        try {
                            const response = await fetch(`https://officeinteriorschennai.com/api/public/api/deleteevent/${item.id}`, {
                                method: 'GET',
                                headers: {
                                    Authorization: `Bearer ${data.token}`,
                                },
        ...
        return visibleData.map((item, index) => (
            <Row key={index}
                data={[
                    ...
                    <Text style={styles.cellText}>{item.event_title}</Text>,
                    ...
                    <Text style={styles.cellText}>{item.event_reason}</Text>,
                    conditionDisplay.includes(data.userrole) ?
                        ...
                        <View style={styles.buttonContainer}>
                            <TouchableOpacity onPress={() => handleView(item)}>
                                <EyeIcon width={15} height={15} color={Black} />
                            </TouchableOpacity>
                        </View>
                ]}
                style={styles.row}
                textStyle={styles.rowText}
                widthArr={[100, 100, 200, 200, 100, 100, 100, 200, 100]}
            />
        ));
    };
    ...
    return (
        <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRequest} />}>
                        ...
                        <ScrollView horizontal>
                            <View style={{ ...styles.tableContainer }}>
                                <Table borderStyle={styles.tableBorder}>
                                    <Row
                                        data={['S.No', 'Title', 'Teams', 'Members', 'Date', 'Start Time', 'End Time', 'Agenda', 'Action']}
